...
#### 連鎖の効率（`w_waste` / `w_junk`、既定 0）

1 段 4 個ちょうどで消える連鎖ほど、同じ広さで長く組めて空きも残る。評価関数の連鎖検出で、その連鎖を撃ったら
何個消えて何個残るかも数え、無駄な連結（消える数 − 連鎖数 × 4）× `w_waste` と、撃った後に残るぷよ × `w_junk` を減点する。
`bench` は「最大連鎖を撃った時点の盤面のぷよ数 / 連鎖数」（1 連鎖あたりのぷよ数、理想は 4）も出す。

| beam_cpp（14 連鎖目標・60 手・100 ゲーム） | 発火率 | 中央値 | 1 連鎖あたりのぷよ数 |
|---|---|---|---|
| 既定 | 32% | 13 | 5.74 |
| `w_waste=300 w_junk=100` | 33% | 12 | 5.58 |

対戦（versus_cpp 同士 60 局）では `w_waste=300 w_junk=100` で 25 勝 35 敗、`extend=1` 併用で 28 勝 32 敗。
減点で誘導しても 1 段 4 個の形はほとんど作れず（組み途中の材料まで「使われないぷよ」として嫌ってしまう）、
効率の良い形を組ませるには定形（階段・GTR など）を教える方が有力。

## 盤面評価ネットワーク（試作）

自分で大量に連鎖を組ませ、その結果を教師にして「この盤面は良いか」を学ぶ。

```sh
python scripts/gen_nn_data.py -n 4000 -o results/nn_data.npz        # beam にとこぷよを打たせて盤面を集める（約 9 分）
python scripts/train_nn.py results/nn_data.npz -o models/eval_mlp_q.bin   # 学習（PyTorch、数十秒）
python -m puyo bench --ai beam_cpp --opt nn=models/eval_mlp_q.bin --opt w_nn=300 ...
```

- 教師データ: ゲームごとに評価の重み・探索の広さを変え、一定の確率でランダムな手も打たせて、良い形も悪い形も集める（4000 ゲーム・約 23 万盤面）
- ラベル（`--label`）: `quality` = その後に撃った最大の連鎖の「連鎖数 − (撃った時点のぷよ数 − 連鎖数 × 4) / 4」（4 個ずつ無駄なく伸ばせた長い連鎖ほど高い、窒息は 0）。
  `chains`（その後に撃った最大の連鎖数）は盤面が埋まるほど高くなり「積むほど良い」と覚えてしまう
- ネットワーク: 各マス × 色の 0/1（420 個）→ 64 → 32 → 1 の全結合。4 色の入れ替えでデータを増やす。
  C++ では置かれているマスの重みを足すだけで 1 層目を計算する（`cpp/src/nn.cpp`、PyTorch と一致をテストで確認）
- 評価関数: 今の評価 × `w_base` ＋ ネットワークの予測 × `w_nn`

| beam_cpp（14 連鎖目標・60 手・100 ゲーム） | 発火率 | 中央値 | 窒息 | 1 連鎖あたりのぷよ数 |
|---|---|---|---|---|
| 今の評価のみ | 32% | 13 | 2% | 5.74 |
| ＋ネットワーク（連鎖数ラベル、`w_nn=1000`） | 8% | 3 | 54% | 6.00 |
| ネットワークのみ（連鎖数ラベル） | 0% | 0 | 99% | – |
| ＋ネットワーク（質ラベル、`w_nn=300`） | 28% | 11.5 | 5% | 6.08 |
| ＋ネットワーク（質ラベル、`w_nn=1000`） | 13% | 10 | 32% | 6.65 |

1 回目は改善しなかった。質ラベルの予測と実際の相関は 0.29 と弱い（ラベルが「その後のツモと打ち方の運」で大きくぶれるため）。
弱い予測に大きな重みを掛けると、同じ局面の候補手を見分ける信号より予測のぶれが勝ち、判断をかき乱す。
...
puyo/
  ...
  tune.py              パラメータ自動調整
cpp/src/nn.cpp          盤面評価ネットワークの推論
scripts/gen_nn_data.py  ネットワークの教師データ作り
scripts/train_nn.py     ネットワークの学習（PyTorch）
models/                 学習したネットワーク
cpp/
